...
function getBinaryImageData(oImg, fncCallback) 
{
	var binaryImg = window.atob(oImg.src.substr(oImg.src.indexOf(',')+1));

	var oEXIF = findEXIFinJPEG(new BinaryFile(binaryImg));
	oImg.exifdata = oEXIF || {};
...
EXIF.getBinaryData = function(oImg, fncCallback) 
{
	if (!oImg.complete) return false;
	if (oImg.src.indexOf('data:') !== 0) return false;

	getBinaryImageData(oImg, fncCallback);
...
jQuery.fn.exifBinaryLoad = function(fncCallback) {
    return this.each(function() {
        EXIF.getBinaryData(this, fncCallback)
    });
}
